...
class SineGen(torch.nn.Module):
    ...
    def _f02sine(self, f0, upp):
        rad = f0 / self.sampling_rate * torch.arange(
            1, upp + 1, device=f0.device
        )
        rad2 = torch.fmod(rad[..., -1:].float() + 0.5, 1.0) - 0.5
        rad_acc = rad2.cumsum(dim=1).fmod(1.0).to(f0)
        # DirectML rejects mixed positive/negative padding. Cropping the final
        # time step first and then padding the beginning is exactly equivalent.
        shifted = F.pad(rad_acc[..., :-1, :], (0, 0, 1, 0))
        rad += shifted
